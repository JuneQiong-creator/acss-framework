...
os.chdir(r"C:\Users\LIU Qiong\Desktop\PhD\research\present\article no.1\code\Carbo+A")
...
df_group1 = pd.DataFrame({
    'ID': [2, 3],
    '400': [0.9934705, 0.953346],
    '0': [0.001285, 0.017677],
    '80': [0.995430667, 0.952431],
    '16': [0.423713, 0.865069],
    '3.2': [0.373334, 0.8223025]
})
...
def plot_all_patients(full_data):
    ...
    plt.title('All Patients Dose-Response Curves for Carbo+A', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig("Carbo+A_all_patients_comparison.png", dpi=300, bbox_inches='tight')
    plt.show()
...
plt.title('Counts of Each Model Being Selected as the Best Model Carbo+A', fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig("Carbo+A_best_model_distribution.png", dpi=300)
plt.show()
...
# Chart decoration
plt.title('Carbo+A Dose-Response Curves (Sigmoid Emax Model)', fontsize=14, pad=20)
plt.xlabel('Dose (μM)', fontsize=12)
plt.ylabel('Response', fontsize=12)
plt.xlim(0, 250)
plt.ylim(-0.05, 1.2)
...
plt.tight_layout()
plt.savefig('Carbo+A_all_patients_best_model_curves.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.suptitle('Carbo+A CRS-ASI Classification Analysis', fontsize=14, y=1.02)
...
plt.savefig(f"Carbo+A_CRS_ASI_Classification_Visualization.png", dpi=300, bbox_inches='tight')
plt.show()

# --- LOOCV visualization ---
if not loocv_results_df.empty:
    plt.figure(figsize=(20,12))
    plt.suptitle('Carbo+A CRS-ASI LOOCV Analysis', fontsize=16, y=1.02)
    ...
    plt.tight_layout(pad=2.0)
    plt.savefig(f"{output_dir}/Carbo+A_CRS_ASI_LOOCV_Analysis_Visualization.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_msscf_grouped_fitted_curves(sigmoid_results, all_patients_df, dose_range=(0, 50), num_points=200):
    ...
    # Chart decorations
    plt.title('Carbo+A Fitted Sigmoid Emax Curves by Grouping (0-50 μM)', fontsize=14, pad=20)
    plt.xlabel('Dose (μM)', fontsize=12)
    plt.ylabel('Response', fontsize=12)
    ...
    # Save plot
    output_path = "Carbo+A_grouped_fitted_curves.png"
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.show()
...
plt.figure(figsize=(20, 6))
plt.suptitle('Carbo+A IC50-CI Classification Analysis', fontsize=14, y=1.02)
...
plt.tight_layout(pad=2.0)
plt.savefig(f"Carbo+A_IC50_CI_Classification_Visualization.png", dpi=300, bbox_inches='tight')
plt.show()

# --- LOOCV Visualization ---
if not loocv_results_df.empty:
    plt.figure(figsize=(20, 12))
    plt.suptitle('Carbo+A IC50-CI LOOCV Analysis', fontsize=16, y=1.02)
    ...
    plt.tight_layout(pad=2.0)
    plt.savefig(f"{output_dir}/Carbo+A_IC50_LOOCV_Analysis_Visualization.png", dpi=300, bbox_inches='tight')
    plt.show()
...
# 1. CRS distribution
plt.figure(figsize=(20,6))
plt.suptitle('Carbo+A CRS-CI Classification Analysis', fontsize=14, y=1.02)
...
plt.tight_layout(pad=2.0)
plt.savefig(f"Carbo+A_CRS_CI_Classification_Visualization.png", dpi=300, bbox_inches='tight')
plt.show()

# --- LOOCV Visualization ---
if not loocv_results_df.empty:
    plt.figure(figsize=(20, 12))
    plt.suptitle('Carbo+A CRS-CI LOOCV Analysis', fontsize=16, y=1.02)
    ...
    plt.tight_layout(pad=2.0)
    plt.savefig(f"{output_dir}/Carbo+A_CRS_LOOCV_Analysis_Visualization.png", dpi=300, bbox_inches='tight')
    plt.show()
...
for (algo_name, model), out_dir in zip(algorithms, output_dirs):
    ...
    plt.tight_layout(pad=2.0)
    plt.savefig(f"Carbo+A_{algo_name}_Clustering_Visualization.png", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    if not loocv_df.empty:
        ...
        plt.tight_layout(pad=2.0)
        plt.savefig(f"{out_dir}/Carbo+A_{algo_name}_LOOCV_Analysis_Visualization.png", dpi=300, bbox_inches='tight')
        plt.show()
...
plt.figure(figsize=(20, 12))
plt.suptitle('Carbo+A Comparison of Clustering Methods Across Metrics', fontsize=16, y=1.02)
...
plt.tight_layout(pad=2.0)
plt.savefig(f"Carbo+A_Comparison_Barplot.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.grid(alpha=0.2)
plt.tight_layout()
plt.savefig(f"Carbo+A_IPES_Ranking_Barplot.png", dpi=300, bbox_inches='tight')
plt.show()
...
plt.ylabel('Method')
plt.tight_layout()
plt.savefig(f"Carbo+A_Comparison_Heatmap.png", dpi=300, bbox_inches='tight')
plt.show()
...
plot_radar(
    ...
    'Carbo+A Radar Plot of Composite Metrics Across Methods',
     f"Carbo+A_Composite_Metrics_Radarplot.png"
)
